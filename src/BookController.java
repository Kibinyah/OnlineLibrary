	...
	public void initialize(URL url, ResourceBundle resources) {
		...
		for(int i = 0; i<Library.resourceList.size(); i++) {
			if(Library.resourceList.get(i).getTitle().trim().equals(finalTit)) {
			    Book someBook = (Book)Library.resourceList.get(i);
			    System.out.println(someBook.copies);
			    year.setText("" + someBook.getYear());
				...
				copies.setText("" + someBook.copies.size());
				for (int j = 0; j < someBook.copies.size(); j++) {
					data.add(someBook.copies.get(j));
				}
			}
