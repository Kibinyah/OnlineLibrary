...
public class DVDController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle resources) {
		...
		String tit = SearchController.selected.substring(6);
		...
		for (int i = 0; i < Library.resourceList.size(); i++) {
			if (Library.resourceList.get(i).getTitle().trim().equals(finalTit)) {
				DVD someDVD = (DVD) Library.resourceList.get(i);
				year.setText("" + someDVD.getYear());
				director.setText(someDVD.getDirector());
				runtime.setText("" + someDVD.getRunTime());
				subtitles.setText(someDVD.getSubtitles());
				language.setText(someDVD.getLanguage());
				copies.setText("" + someDVD.copies.size());
				for (int j = 0; j < someDVD.copies.size(); j++) {
					data.add(someDVD.copies.get(j));
				}
			}
